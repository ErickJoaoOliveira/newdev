...
exports.post = async(req, res) => {
  try {
    if (!req.body.name || !req.body.urlAvatar) {
...
exports.getId = async(req, res) => {
  try {
    ...
    return res.status(200).send({name: professor.name, urlAvatar: professor.urlAvatar});
  } catch (error) {
    logger(error.message);
    return res.status(500).send({error: error?.message || e});
  }
...
exports.put = async(req, res) => {
  try {
    ...
    await database.update({name: nextProfessor.name, urlAvatar: nextProfessor.urlAvatar}).from('professors').where({id: previousProfessor.id});
